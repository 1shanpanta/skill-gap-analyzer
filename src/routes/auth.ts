...
const authLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { error: 'TooManyRequests', message: 'Too many attempts, please try again later', statusCode: 429 },
});
...
router.post('/forgot-password', strictLimiter, async (req, res, next) => {
  try {
    ...
    if (err instanceof z.ZodError) {
      res.status(400).json({ error: 'ValidationError', message: 'Valid email required', statusCode: 400 });
      return;
    }
    next(err);
  }
...
router.post('/reset-password', strictLimiter, async (req, res, next) => {
  try {
    ...
    if (err instanceof z.ZodError) {
      res.status(400).json({ error: 'ValidationError', message: 'Password must be 8+ chars with a letter and number', statusCode: 400 });
      return;
    }
    next(err);
  }
...
router.post('/change-password', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!user.password_hash) {
      throw new AppError(400, 'Account uses Google sign-in. Set a password via forgot password first.');
    }
...
router.delete('/unlink-google', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!user.password_hash) {
      throw new AppError(400, 'Cannot unlink Google — set a password first to keep access to your account.');
    }
